...
export const setupRecording = (
  ...
  const codecConfigs = [
    // Chrome/Edge preferred (VP9 with Opus)
    {
      mimeType: "video/webm;codecs=vp9,opus",
      audioBitsPerSecond: 128000,
      videoBitsPerSecond: 2500000,
    },
    // Firefox preferred (VP8 with Opus)
    {
  ...
  // Try each codec configuration until one works
  for (const config of codecConfigs) {
    try {
      ...
      console.warn(`Codec ${config.mimeType} not supported, Error: ${error}`);
      continue;
    }
  }
...
export const createIframeLink = (videoId: string) =>
  `https://iframe.mediadelivery.net/embed/501560/${videoId}?autoplay=true&preload=true`;
